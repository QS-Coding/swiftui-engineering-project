...
class PostService {
    ...
    static func createPost(message: String, image: UIImage?, token: String) async throws -> Bool {
        if let image = image {
    ...
    static private func createPostWithImage(message: String, imgUrl: String?, token: String) async throws -> Bool {
        guard let url = URL(string: "\(baseURL)/posts") else {
            throw URLError(.badURL)
        }
    ...
    static func updateLikes(postId: String, token: String) async throws -> Bool {
